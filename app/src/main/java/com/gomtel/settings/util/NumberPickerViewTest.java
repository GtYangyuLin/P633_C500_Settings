 ...
 */
public class NumberPickerViewTest extends View {
    ...
    private static int mItemHigh = 0;

    // compatible for NumberPicker
    public interface OnValueChangeListener {
        void onValueChange(NumberPickerViewTest picker, int oldVal, int newVal);
    }

    public interface OnValueChangeListenerRelativeToRaw {
        void onValueChangeRelativeToRaw(NumberPickerViewTest picker, int oldPickedIndex, int newPickedIndex,
                                        String[] displayedValues);
    }

    public interface OnValueChangeListenerInScrolling {
        void onValueChangeInScrolling(NumberPickerViewTest picker, int oldVal, int newVal);
    }
    ...
    public interface OnScrollListener {
        ...
        void onScrollStateChange(NumberPickerViewTest numberPickerView, int scrollState);
    }
    ...
    public NumberPickerViewTest(Context context) {
        super(context);
        init(context);
    }

    public NumberPickerViewTest(Context context, AttributeSet attrs) {
        super(context, attrs);
        initAttr(context, attrs);
        init(context);
    }

    public NumberPickerViewTest(Context context, AttributeSet attrs, int defStyleAttr) {
                    ...
                    case HANDLER_WHAT_REFRESH:
                        if (!mScroller.isFinished()) {
                            ...
                            if (mCurrDrawFirstItemY != 0) {//need to adjust
                                ...
                                //startposition = mCurrDrawGlobalY;
                                mItemHigh = mItemHeight;
    ...
    protected void onDetachedFromWindow() {
        ...
        if (!mScroller.isFinished()) {
            ...
            Log.e("yangyltest", "onDetachedFromWindow: mCurrDrawGlobalY======" + mCurrDrawGlobalY);
            ...
            if (mCurrDrawFirstItemY != 0) {
                if (mCurrDrawFirstItemY < (-mItemHeight / 2)) {
                    ...
                    Log.e("yangyltest", "onDetachedFromWindow: mCurrDrawGlobalY===if===" + mCurrDrawGlobalY);
                } else {
                    mCurrDrawGlobalY = mCurrDrawGlobalY + mCurrDrawFirstItemY;
                    Log.e("yangyltest", "onDetachedFromWindow: mCurrDrawGlobalY===else===" + mCurrDrawGlobalY);
                }
                calculateFirstItemParameterByGlobalY();
            }
            onScrollStateChange(OnScrollListener.SCROLL_STATE_IDLE);
        ...
        if (currPickedIndex != mPrevPickedIndex && mRespondChangeOnDetach) {
            try {
                if (mOnValueChangeListener != null) {
                    mOnValueChangeListener.onValueChange(NumberPickerViewTest.this, mPrevPickedIndex + mMinValue, currPickedIndex + mMinValue);
                }
                if (mOnValueChangeListenerRaw != null) {
                    mOnValueChangeListenerRaw.onValueChangeRelativeToRaw(NumberPickerViewTest.this, mPrevPickedIndex, currPickedIndex, mDisplayedValues);
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    private void respondPickedValueChanged(int oldVal, int newVal, Object respondChange) {
        ...
        if (oldVal != newVal) {
            if (respondChange == null || !(respondChange instanceof Boolean) || (Boolean) respondChange) {
                if (mOnValueChangeListener != null) {
                    mOnValueChangeListener.onValueChange(NumberPickerViewTest.this, oldVal + mMinValue, newVal + mMinValue);
                }
                if (mOnValueChangeListenerRaw != null) {
                    mOnValueChangeListenerRaw.onValueChangeRelativeToRaw(NumberPickerViewTest.this, oldVal, newVal, mDisplayedValues);
                }
            }
        }
    ...
    public static int getDateCircleNumber() {
        int mCircleNumber = (stopposition - startposition) / mItemHigh;
        if (mCircleNumber > 0) {
            mCircleNumber = (int) Math.ceil(mCircleNumber);
            return mCircleNumber;
        } else {
            mCircleNumber = (int) Math.floor(mCircleNumber);
            return mCircleNumber;
        }
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                startposition = mCurrDrawGlobalY;
                Log.e("yang", "onTouchEvent: mCurrDrawGlobalY========" + mCurrDrawGlobalY);
                onScrollStateChange(OnScrollListener.SCROLL_STATE_IDLE);
    ...
    private void correctPositionByDefaultValue(int defaultPickedIndex, boolean wrap) {
        Log.e("yang", "correctPositionByDefaultValue: defaultPickedIndex====" + defaultPickedIndex);
    ...
    public void computeScroll() {
        ...
        if (mScroller.computeScrollOffset()) {
            mCurrDrawGlobalY = mScroller.getCurrY();
            stopposition = mCurrDrawGlobalY;
            Log.e("yang", "computeScroll: mCurrDrawGlobalY++++++" + mCurrDrawGlobalY);
            calculateFirstItemParameterByGlobalY();
            postInvalidate();
        }
    }
    ...
    private void drawContent(Canvas canvas) {
        ...
        for (int i = 0; i < mShowCount + 1; i++) {
            ...
            Log.e("yang", "drawContent: i====y====y"+y);
            Log.e("yang", "drawContent: i====y====mCurrDrawFirstItemY"+mCurrDrawFirstItemY);
            ...
            if (i == mShowCount / 2) {//this will be picked
                ...
                Log.e("yang", "drawContent: i====else=if==="+i);
                ...
                Log.e("yang", "drawContent: i====else====");
                textColor = mTextColorNormal;
                textSize = mTextSizeNormal;
                textSizeCenterYOffset = mTextSizeNormalCenterYOffset;
                Log.e("yang", "drawContent: i====else====mTextSizeNormalCenterYOffset"+mTextSizeNormalCenterYOffset);
            }
            ...
            if (0 <= index && index < getOneRecycleSize()) {
                ...
                if (mTextEllipsize != null) {
                    Log.e("yang", "drawContent: ====if=2===");
                    str = TextUtils.ellipsize(str, mPaintText, getWidth() - 2 * mItemPaddingHorizontal, getEllipsizeType());
                }
                Log.e("yang", "drawContent: ====if=1===");
                //drawRoller(canvas, i, str, y, textSizeCenterYOffset);
                ...
                Log.e("yang", "drawContent: ====else=if===");
    ...
    private void drawRoller(Canvas canvas, int i, CharSequence str, float y, float textSizeCenterYOffset) {
        int halfCircumference = (int) (getMeasuredHeight() * Math.PI / 2);
        int maxTextHeight = (int) (halfCircumference / (2F * (mShowCount - 1)));
        Log.e("yangyl", "drawRoller: mShowCount======" + mShowCount);
        float itemHeight = maxTextHeight * 2F;
        int radius = getMeasuredHeight() / 2;
        int j2 = (int) ((stopposition - startposition) % (2F * maxTextHeight));
        ...
        if (radian >= Math.PI || radian <= 0) {
            ...
            Log.e("yangyl", "drawRoller: radius======" + radius);
            Log.e("yangyl", "drawRoller: maxTextHeight======" + maxTextHeight);
